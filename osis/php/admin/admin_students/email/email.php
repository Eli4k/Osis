...
$message = "";
	...
	foreach($_POST["sid"] as $record)
	{
		$mail = new PHPMailer\PHPMailer\PHPMailer();
		$sql = $con->query("SELECT * FROM students WHERE sid = '".$record."'");

		foreach($sql as $info)
		{
	
			$body = '<p>Hello '.$info["last_name"].', your Information Portal account has been successfully created. Kindly visit <a href="localhost/schoolportal">www.gaecsip.com</a> and Enter to Your Login credentials.</p>
				<p>Your Password and User Id are added in the message below</p>
				<p>Password: '.$info["password"].'<br>
				   User Id: '.$info["student_id"].'</p>
				<p>GAEC BASIC SCHOOL always with the best</p>';
								...
								if($mail->send())
								{
									$con->query("UPDATE students SET notified = 1 WHERE sid = '".$info["sid"]."'");
