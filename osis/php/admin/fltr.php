...
if(isset($_GET)){
  ...
  if($level > 0 && $stream < 1){
    $sql = $con->query("SELECT sid, student_id, c.c_id, st.s_id, first_name, last_name, password, cname, contact, admission_date, s_name, email
                    FROM class c INNER JOIN students s
                    ON s.c_id = c.c_id
                    JOIN stream st
                     ON s.s_id = st.s_id
                    WHERE s.c_id = '$level' ORDER BY s.c_id");
  }
  elseif($level < 1 && $stream > 0){
    $sql = $con->query("SELECT sid, student_id, c.c_id, first_name, last_name, password, cname, contact, admission_date, s_name, email
                    FROM class c INNER JOIN students s
                    ON s.c_id = c.c_id
                    JOIN stream st
                     ON s.s_id = st.s_id
                    WHERE st.s_id = '$stream' ORDER BY s.c_id");
  }
  else{
    $sql = $con->query("SELECT sid, student_id, c.c_id, first_name, last_name, password, cname, contact, admission_date, s_name, email
                  FROM class c INNER JOIN students s
                  ...
                  JOIN stream st
                   ON s.s_id = st.s_id
                  WHERE st.s_id = '$stream' AND s.c_id = '$level' ORDER BY last_name");
            }
  ...
  if($total > 0){
    while($row = $sql->fetch_assoc())
    {
        $output.= ' <tr class="gradeX">
                       ...
                       <td>
                           <button id="btn_view" title="View Profile"  class="btn btn-secondary btn-mini" data-view="'.$row['sid'].'">
                             <i class="icon-user"></i></span></button>
                               <button title="Edit" class="btn btn-secondary btn-mini" data-edit="'.$row['sid'].'">
                                 <i class="icon-edit"></i></span></button>
                             <button title="Delete" class="btn btn-secondary btn-mini" data-delete="'.$row['sid'].'">
